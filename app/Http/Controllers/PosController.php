   ...
   public function index(Request $request)
{
    ...
    if ($user_auth->can('pos')) {
        ...
        if ($settings->payment_method_id) {
            $paymentMethod = PaymentMethod::where('id', $settings->payment_method_id)
                ->where('deleted_at', '=', null)
                ->first();
                
            if ($paymentMethod) {
                $default_payment_method = $settings->payment_method_id;
            }
        }

        // If no default payment method is set, get the first available one
        if (empty($default_payment_method)) {
            $firstPaymentMethod = PaymentMethod::where('deleted_at', '=', null)
                ->orderBy('id', 'desc')
                ->first();
      ...
      // Default Client Setup - Improved logic
        $default_Client = $settings->client_id ?? '';
        if ($default_Client && !Client::where('id', $default_Client)->exists()) {
            $default_Client = '';
        }

        // If no default client, get the first available one
        if (empty($default_Client)) {
            $firstClient = Client::where('deleted_at', '=', null)
                ->orderBy('id', 'desc')
                ->first();
            $default_Client = $firstClient->id ?? '';
        }
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            if ($product_warehouse->product_variant_id) {
               ...
               $item['cost'] = $product_warehouse['product']->cost ;
                $item['takeaway'] = $product_warehouse['product']->cost ;
               ...
               $item['name'] = $product_warehouse['product']->name;
                $item['takeaway'] = $product_warehouse['product']->name;
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            if ($product_warehouse->product_variant_id) {
                ...
                $item['barcode'] = '['.$product_warehouse['productVariant']->name . '] ' . $product_warehouse['product']->name;
                  $item['cost'] = $product_warehouse['product']->cost ?? false; 
     ...
     public function Print_Invoice_POS(Request $request, $id)
{
    ...
    if ($user_auth->can('pos')) {
        ...
        return view('sales.invoice_pos', [
            ...
            'setting' => $settings,
            'pos_settings' => array_merge($pos_settings->toArray(), [
                'is_printable' => $request->has('autoprint')
            ]),
            'sale' => $item,
            'details' => $details,
        ]);
...
public function Print_Invoice_POS_Kitchen(Request $request, $id)
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        // You can retrieve the created order again if needed
        $orderRef = 'SO-' . date("Ymd") . '-' . date("his");
