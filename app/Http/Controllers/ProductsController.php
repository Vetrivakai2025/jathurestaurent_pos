...
class ProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
		...
		if ($user_auth->can('products_add')){
             ...
             $productRules = [
                ...
                'takeaway'     => Rule::requiredIf($request->type == 'is_single'),
            ];
            ...
            if ($request->type == 'is_variant') {
                $productRules['variants'] = [
                    ...
                    function ($attribute, $value, $fail) use ($request) {
                        ...
                        if($variants){
                            foreach ($variants as $variant) {
                                if (!array_key_exists('text', $variant) || empty($variant['text'])) {
                                    ...
                                    return;
                                }else if(!array_key_exists('takeaway', $variant) || empty($variant['takeaway'])) {
                                    $fail('Variant takeaway cannot be empty.');
                                    return;
                      ...
                      $all_takeaway = array_column($variants, 'takeaway');
                        if($all_takeaway){
                            foreach ($all_takeaway as $cost) {
                                if (empty($cost)) {
                                    $fail('Variant Takeaway cannot be empty.');
                                    return;
                                }
    ...
    public function show_product_data($id , $variant_id)
    {
        ...
        if($Product_data['type'] == 'is_single'){
            ...
            $product_takeaway  = $Product_data['takeaway'];

            $item['code'] = $Product_data['code'];
            $item['takeaway'] = $Product_data['takeaway'];
        ...
        //product is_variant
            ...
            $product_takeaway = $product_variant_data['takeaway'];
            $product_cost  = $product_variant_data['cost'];
            $item['code'] = $product_variant_data['code'];
            $item['takeaway'] = $product_variant_data['takeaway'];
